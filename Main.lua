-- Watch EGG | DORO - Standalone Main
-- UI library + Drawer Menu + all Steal an Egg features are self-contained in this file.
-- No ScriptLoader or Window1.lua dependency is required.
...
function Library:CreateWindow(opts)
    ...
    -- Optional vertical drawer menu. Tabs are placed here when DrawerMenu=true.
    local drawerScroll = make("ScrollingFrame", {
        Name="DrawerMenu",
        ...
        Visible=opts.DrawerMenu == true, Parent=sidebar,
    })
    local drawerInner = make("Frame", {
        Name="DrawerInner", Size=UDim2.new(1,-4,0,0),
    ...
    make("UIListLayout", {
        ...
        SortOrder=Enum.SortOrder.LayoutOrder,
        Padding=UDim.new(0,6), Parent=drawerInner,
    })
    ...
    windowRef = setmetatable({
        ...
        _logoAsset=logoAsset, _hotbar=hotbar, _hotbarInner=hotbarInner,
        _drawerMenu=(opts.DrawerMenu == true), _drawerScroll=drawerScroll, _drawerInner=drawerInner, _content=content,
...
function Window:AddTab(opts)
    ...
    local drawerMode = self._drawerMenu == true and self._drawerInner ~= nil
    local tabParent = drawerMode and self._drawerInner or self._hotbarInner
    local hBtn=make("TextButton",{
        Text="", AutomaticSize=not drawerMode,
        Size=drawerMode and UDim2.new(1,0,0,40) or UDim2.new(0,0,1,0),
        BackgroundColor3=C.HotbarBg,
        ZIndex=5, Parent=tabParent,
    })
    hBtn.LayoutOrder=#tabParent:GetChildren()
    corner(hBtn,7); pad(hBtn,0,0,12,12)
    table.insert(win._noDrag,hBtn)

    local hRow=make("Frame",{
        BackgroundTransparency=1, AutomaticSize=not drawerMode,
        Size=drawerMode and UDim2.new(1,0,1,0) or UDim2.new(0,0,1,0),
        ZIndex=5, Parent=hBtn,
    })
    ...
    local hLabel=make("TextLabel",{
        ...
        AutomaticSize=not drawerMode,
        Size=drawerMode and UDim2.new(1,-32,1,0) or UDim2.new(0,0,1,0),
        TextXAlignment=drawerMode and Enum.TextXAlignment.Left or Enum.TextXAlignment.Center,
        LayoutOrder=2,ZIndex=6,Parent=hRow,
    })

    local hDot=make("Frame",{
        AnchorPoint=drawerMode and Vector2.new(1,0.5) or Vector2.new(0.5,0),
        Position=drawerMode and UDim2.new(1,-8,0.5,0) or UDim2.new(0.5,0,1,4),
...
local function Notify(title, content, kind, dur)
    pcall(function()
        MainWindow:Notify({ Title = title, Content = content, Type = kind or "Info", Duration = dur or 2.5 })
    end)
end
...
local function safeCallback(fn)
    return function(...)
        ...
        if not ok then
            pcall(Notify, "Watch EGG ", "Error: " .. tostring(err), "Error", 4)
        end
    end
end
-- ==============================================================================
...
task.spawn(function()
    while not HUB.dead do
        if masterEnabled and autoStealEnabled then
...
task.spawn(function()
    while not HUB.dead do
        if masterEnabled and autoHatchEnabled then
            pcall(HatchAllReadyEggs)
        end
        if masterEnabled and autoPlantEnabled then
            pcall(PlantAllCarriedEggsInPen)
        end
...
task.spawn(function()
    ...
    while not HUB.dead do
        if masterEnabled and batAuraEnabled and batRe then
            local hrp = findHRP()
            if hrp then
...
task.spawn(function()
    ...
    while not HUB.dead do
        if masterEnabled and (avoidTrapsEnabled or autoStealEnabled) then
...
-- ==============================================================================
local EggsTab     = MainWindow:AddTab({ Name = "Eggs", Subtitle = "Steal, hatch & plant", Icon = "crown" })
local BaseTab     = MainWindow:AddTab({ Name = "Base", Subtitle = "Homestead & training", Icon = "bolt" })
local CombatTab   = MainWindow:AddTab({ Name = "Combat", Subtitle = "Bat, slaps & defense", Icon = "combat" })
local PlayerTab   = MainWindow:AddTab({ Name = "Player", Subtitle = "Movement & teleports", Icon = "player" })
local SettingsTab = MainWindow:AddTab({ Name = "Settings", Subtitle = "Configs & unloader", Icon = "gear" })
...
ConfigSub:AddToggle({
    Name = "Master Features Toggle", Default = true, Flag = "master_features",
    Callback = safeCallback(function(v)
        ...
        Notify("Master Features", masterEnabled and "Master switch ON — individual toggles are active" or "Master switch OFF — active automations were disabled", masterEnabled and "Success" or "Warning")
    end)
})
